...
function CsbApp(canvasElementName, isDebugging) {
    ...
    this.startApp = function(applicationName) {
        ...
        setInterval(function() {self.drawCanvas.call(self)}, 10);
    ...
    this.loginUser = function() {
        var self = this;
        $.post('/auth/ajax_login', {'username-or-email': $("#app-login-username-or-email").val(), 'password': $("#app-login-password").val()}, function(result) {
    ...
    this.getTutorialsFromCookie = function() {
        ...
        cookies = cookies.filter(function(cookie) {return cookie.indexOf("tutorials_complete") >= 0;});
        if (cookies.length == 0)
            return [];
